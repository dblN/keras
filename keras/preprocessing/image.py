...
class ImageDataGenerator(object):
    ...
    def flow_from_file_list(self, filepaths, labels,
                            target_size=(256, 256), color_mode='rgb',
                            classes=None, class_mode='categorical',
                            batch_size=32, shuffle=True, seed=None,
                            save_to_dir=None,
...
class FileIterator(Iterator):
    ...
    def __init__(self, filepaths, labels, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 dim_ordering='default',
                 classes=None, class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
                 save_to_dir=None, save_prefix='', save_format='jpeg'):
        """
        :param filepaths: ["directory/image.png", ...]
        :param labels: [0, 9, 1, ...]
        """
        ...
        super(FileIterator, self).__init__(self.nb_sample, batch_size, shuffle, seed)
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                fname = '{prefix}_{index}_{hash}.{format}'.format(prefix=self.save_prefix,
                                                                  ...
                                                                  hash=np.random.randint(1e4),
                                                                  format=self.save_format)
                img.save(os.path.join(self.save_to_dir, fname.split("/")[-1]))
        # build batch of labels
        if self.class_mode == 'sparse':
            batch_y = self.classes[index_array]
        elif self.class_mode == 'binary':
